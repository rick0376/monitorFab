...
export default function MachineSection() {
  ...
  const [maquinasAgrupadas, setMaquinasAgrupadas] = useState({});
  ...
  useEffect(() => {
    async function fetchAndGroup() {
      ...
      const agrup = {};
      ordenadas.forEach((m) => {
        const key = m.secaoId || "sem-secao";
        if (!agrup[key]) {
          agrup[key] = {
            nome: m.secao?.nome || "Sem Seção",
            maquinas: [],
          };
        }
        agrup[key].maquinas.push(m);
      });
      setMaquinasAgrupadas(agrup);
    }
    fetchAndGroup();
    const timer = setInterval(fetchAndGroup, refreshInterval);
  ...
  const openModal = (maquina) => {
    ...
    setStartDate(formattedOntem);
    setStartTime("00:01");
    setEndDate(formattedHoje);
    setEndTime("23:59");
  ...
  const filterParadasByDate = () => {
    if (!selectedMachine?.paradas) return [];
    const s = buildStart();
    const e = buildEnd();
    return selectedMachine.paradas.filter((p) => {
      const dt = new Date(p.horaInicio);
      if (s && e) return isWithinInterval(dt, { start: s, end: e });
      if (s) return isSameDay(dt, startOfDay(s));
      return true;
    });
  };

  const getFilteredParadas = () => {
    const list = filterParadasByDate();
  ...
  const handleEditParada = (id) => {
    router.push({
      ...
      query: { id, returnUrl: "/painel/controlmaquina/secao" },
    });
  };
  ...
  return (
    <div className={styles.body}>
      ...
      <div className={styles.container}>
        {Object.values(maquinasAgrupadas).map((secao) => (
          <div key={secao.nome} style={{ marginBottom: 16 }}>
            <h2 className={styles.sectionTitle}>{secao.nome}</h2>
              ...
              {secao.maquinas.map((m) => {
                ...
                return (
                  ...
                  >
                    <p>{m.nome}</p>
                    <p>
                      Status: {m.funcionando ? "Funcionando" : "Em Manutenção"}
                    </p>
                    {ativo && (
                      <p>Tempo: {formatTime(machineTimers[m.id] || 0)}</p>
                    )}
                  </div>
                );
              })}
